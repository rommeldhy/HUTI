...
class BPR_P_Loss(nn.Module):
    """ BPR with predictability, modified from BPRLoss.
    ...
    def __init__(self, gamma=1e-10):
        super(BPR_P_Loss, self).__init__()
        self.gamma = gamma

    def forward(self, pos_score, neg_score, p):
        loss = -(torch.log(self.gamma + torch.sigmoid(pos_score - neg_score)) * p).mean() * torch.ones_like(p).sum() / (p.sum())
        return loss

class CE_P_Loss(nn.Module):
    """ Cross Entropy with predictability"""
    
    def __init__(self):
        super(CE_P_Loss, self).__init__()
        self.CE = nn.CrossEntropyLoss(reduction='none')
        
    def forward(self, logits, pos_items, p):
        loss = (self.CE(logits, pos_items) * p).mean() * torch.ones_like(p).sum() / (p.sum())
        return loss
